    ...
    if web_element:
        if 'parent' in kwargs and kwargs['parent']:
            ...
            web_element = element.get_element_from_childnodes(
                web_element, tag_name, dom_traversing=False)[int(index)]
...
def find_text(text: str) -> bool:
    try:
        if javascript.execute_javascript("return window.find('{}')".format(
                text.replace("\'", "\\'"))):
            return True
    except WebDriverException as e:
        logger.debug(
            'Got webdriver exception from find text func: {}'.format(e))
...
def get_text_elements(text: str, **kwargs) -> Optional[list[WebElement]]:
    ...
    if shadow_dom:
        shadow_elements = get_texts_including_shadow_dom(
            text, partial, **kwargs)
...
def get_all_text_elements(text: str, **kwargs) -> list[WebElement]:
    ...
    all_text_nodes = util.par2bool(
        kwargs.get('all_text_nodes', CONFIG['AllTextNodes']))
    kwargs['partial_match'] = kwargs.get('partial_match',
                                         CONFIG['PartialMatch'])
    if all_text_nodes:
...
def _filter_by_modal_ancestor(elements: list[WebElement]) -> list[WebElement]:
    ...
    for elem in elements:
        try:
            ...
            logger.debug(
                "Filtering out element, modal open but not under modal")
            continue
...
def get_element_using_anchor(elements: list[WebElement],
                             anchor: Optional[Union[str, int]],
                             **kwargs) -> WebElement:
    ...
    if str(anchor).isdigit() and kwargs.get("anchor_type",
                                            "auto").lower() != "text":
...
def get_item_using_anchor(text: str, anchor: str,
    ...
    if no_raise:
        return None
    raise QWebElementNotFoundError(
        'Cannot find item for locator {}'.format(text))


def _get_correct_element(web_elements: list[WebElement], anchor: str,
                         **kwargs) -> WebElement:
    if len(web_elements) == 1:
        return web_elements[0]
    correct_element = get_element_using_anchor(web_elements, anchor, **kwargs)
...
@frame.all_frames
def get_texts_including_shadow_dom(locator: str, partial: bool,
                                   **kwargs) -> list[WebElement]:
    web_elements = element.get_visible_elements_from_elements(
        javascript.get_text_elements_from_shadow_dom(locator, partial),
        **kwargs)
    if web_elements:
...
@frame.all_frames
def get_items_including_shadow_dom(text: str, tag: str,
                                   **kwargs) -> list[WebElement]:
    web_elements = element.get_visible_elements_from_elements(
    ...
    if shadow_elements:
        logger.debug(
            f'Found {len(shadow_elements)} items when extending search to shadow dom'
        )
