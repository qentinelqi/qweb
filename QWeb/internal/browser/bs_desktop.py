...
def open_browser(bs_browser: str, project_name: str, run_id: str) -> WebDriver:
    ...
    desired_caps = {
        ...
        bs_os,
        'resolution':
        BuiltIn().get_variable_value('${BSRESOLUTION}') or '1920x1080',
        'browserstack.local':
        BuiltIn().get_variable_value('${BSLOCAL}') or 'false',
        'browserstack.localIdentifier':
        BuiltIn().get_variable_value('${BSLOCALID}') or '',
        'os_version':
        BuiltIn().get_variable_value('${BSOSVERSION}') or OS[bs_os.lower()],
        'browser':
        BuiltIn().get_variable_value('${BROWSER}') or NAMES[bs_browser][0],
        'browser_version':
        BuiltIn().get_variable_value('${BROWSERVERSION}')
        or NAMES[bs_browser][1]
    }

    try:
        executor_url = 'https://{}:{}@hub-cloud.browserstack.com/wd/hub'.format(
            bs_user, bs_key)
